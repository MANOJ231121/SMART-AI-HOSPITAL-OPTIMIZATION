    ...
    >
      <Card className="w-full max-w-lg p-8 text-center shadow-md">
        ...
        {patientData?.extracted?.["Main Symptom"] && (
          ...
          >
            <div><strong>Reported Complaint:</strong> {patientData.extracted["Main Symptom"]}</div>
            {patientData.extracted["Duration"] && <div><strong>Duration:</strong> {patientData.extracted["Duration"]}</div>}
            {patientData.extracted["Severity"] && <div><strong>Severity:</strong> {patientData.extracted["Severity"]}</div>}
          </div>
        )}
        ...
        >
          <p className="text-xs font-semibold" style={{ color: COLORS.inkSoft }}>
            This is an AI-assisted department recommendation,
            not a medical diagnosis. Final medical decisions
