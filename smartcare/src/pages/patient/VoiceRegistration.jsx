...
import { getFollowUpsForComplaint, getComplaintLabel } from "../../data/symptomFollowUps";
...
export default function VoiceRegistration({
  ...
  const [scriptSteps, setScriptSteps] = useState(() => getRegistrationScript(lang));
  const step = scriptSteps[stepIdx] || scriptSteps[0];

  // Refresh script when language changes
  useEffect(() => {
    const newScript = getRegistrationScript(lang);
    setScriptSteps(newScript);
    setStepIdx(0);
    setExtracted({});
    setTranscript([]);
  }, [lang]);
  
  // Speak the current question whenever step changes
  useEffect(() => {
    if (!audioUnlocked || !step) return;
    if (muted) {
      setMicState("idle");
      return;
    }
    ...
    speak(step.q, ttsLang, {
      onEnd: () => setMicState("idle"),
    });
    return () => stopSpeaking();
  }, [stepIdx, muted, audioUnlocked, step, lang]);
  ...
  function submitAnswer(answer) {
    ...
    if (step?.extractLabel && typeof step?.extract === "function") {
      let value = step.extract(cleanAnswer);

      if (step.key === "complaint") {
        value = getComplaintLabel(cleanAnswer);
      }

      newExtracted[step.extractLabel] = value;
    }

    // Multilingual emergency screening detection
    if (
      step?.key === "complaint" &&
      /chest pain|breathless|saans|faint|seene me dard|chhati|hriday|dhabkara|shwas|છાતી|શ્વાસ|હૃદય|ચક્કર|બેભાન|લોહી/i.test(cleanAnswer)
    ) {
    ...
    // Right after complaint, dynamically inject language-specific follow-up questions
    let updatedSteps = scriptSteps;
    if (step?.key === "complaint") {
      const { steps: followUps, preDuration, preSeverity } =
        getFollowUpsForComplaint(cleanAnswer, lang);

      if (preDuration) newExtracted["Duration"] = preDuration;
      if (preSeverity) newExtracted["Severity"] = preSeverity;
      updatedSteps = [
        ...scriptSteps.slice(0, stepIdx + 1),
        ...followUps,
      ];
  ...
  async function handleTap() {
    ...
    if (micState !== "idle" || window.speechSynthesis.speaking) {
      return;
    }
